;;; nbody, configuration 1: the floor.
;;;
;;; Portable R5RS. Generic arithmetic, plain `vector`, no escape hatch of any
;;; kind, because R5RS contains none: no declarations, no type-specific
;;; operators, no policy switch.
;;;
...
;;; sense than intended.
;;;
;;; N is read from stdin rather than argv, because R5RS has neither a
;;; command-line nor an environment accessor and `read` is the only channel the
;;; standard provides.
;;;
;;; Chez:   echo 1000 | scheme --script config1.scm
;;; Racket: echo 1000 | racket config1.scm      (with the #lang line below)
;;;
