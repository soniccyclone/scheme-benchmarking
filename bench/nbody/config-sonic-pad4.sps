...
;; FOUR slots per body in `pos` and `vel`, one in `mass`, and the fourth slot is
;; PADDING THE PROGRAM OWNS. Identical physics to config-sonic.sps -- the same
;; values in the same order -- laid out with a stride of 4 instead of 3.
;;
;; It exists because slp.ss's four-lane packing is complete and unreachable. Its
;; own comment says so: "Off until a padded layout exists to point it at --
;; nbody stores bodies three-wide, so seeding four adjacent stores finds nothing
;; there." Its `store-at` needs four stores sharing a base AND an index vreg,
;; differing only in offset, and one iteration of a three-wide position update
;; writes exactly three. The fourth element belongs to the next body, under a
;; different index.
;;
;; With a stride of 4 the fourth element is padding rather than the next body's
;; x, so there is nothing to mask and four adjacent stores exist within one
;; iteration -- which is the shape vec-x86-64.ss's four-lane row was written for
;; and D59 removed the unconditional AVX-512 prologue in service of.
;;
;; THE ANSWER MUST NOT CHANGE. Same arithmetic, same order, same roundings; only
;; the addresses differ. If the energies move, the transformation is wrong and
;; not the layout. See bead 1mp.5.
(define pos (make-flvector 20 0.0))
(define vel (make-flvector 20 0.0))
    ...
    (let loop ((i 0))
      (when (fx< i n-bodies)
        (let ((b (fx* i 4)))
          ...
          ;; THE PAD IS WRITTEN, and that is the whole point of this variant.
          ;;
          ;; A padded LAYOUT is not sufficient on its own -- measured. slp.ss
          ;; seeds a four-lane pack from four stores sharing a base and an index
          ;; vreg with consecutive offsets, and a program that merely leaves slot
          ;; 3 alone still emits three. The pad has to be STORED for the fourth
          ;; adjacent store to exist.
          ;;
          ;; vel's pad is 0.0 and never read, so this computes 0 + dt*0 into a
          ;; slot nothing consumes: it cannot change the energies, and the
          ;; oracle check is what says so.
          (flvector-set! p (fx+ b 3)
                         (fl+ (flvector-ref p (fx+ b 3))
                              (fl* dt (flvector-ref v (fx+ b 3))))))
        (loop (fx+ i 1))))))
