;;; nbody, configuration 2c: predicate-guarded, at optimize-level 2.
;;;
;;; The experiment PLAN.md section 5 asks for. Chez's cptypes.ss has
;;; fold-primref/try-unsafe (line 1963), which promotes a safe primitive to its
;;; unsafe twin once the argument types check out, for 270 primitives marked
;;; safeongoodargs. Predicate tests are the one user-visible way to feed that
;;; lattice.
;;;
;;; So: can a portable-looking program with predicate guards reach
;;; optimize-level 3 performance while staying at level 2 and staying safe?
;;;
;;; The prediction from CHEZ-ANALYSIS.md is no, and specifically HALF. The
;;; guards should let cptypes drop the TYPE check on flvector-ref, because
;;; flvector? is exactly the predicate its lattice understands. They cannot
;;; drop the BOUNDS check, because cptypes-lattice.ss is a level-1 lattice of
;;; categories that collapses index, length and sub-index into fixnum-pred and
;;; therefore cannot represent i in [0,35).
;;;
...
;;; pragma Suppress can.
;;;
;;; Written from SPEC.md. Expression order matches ref.c exactly, and is
;;; byte-identical to config4-chez.ss apart from the guards.
...
(define (advance!)
  (unless (flvector? b) (error 'advance! "not an flvector"))
  (let outer ([i 0])
    (when (fx< i nbody)
      (let inner ([j (fx+ i 1)])
        (when (fx< j nbody)
...
(define (kinetic i)
  (unless (and (flvector? b) (fixnum? i)) (error 'kinetic "bad"))
  (fl* (fl* 0.5 (g i 6))
       (fl+ (fl+ (fl* (g i 3) (g i 3)) (fl* (g i 4) (g i 4)))
            (fl* (g i 5) (g i 5)))))

(define (pair-potential i j)
  (unless (and (flvector? b) (fixnum? i) (fixnum? j)) (error 'pp "bad"))
  (let* ([dx (fl- (g i 0) (g j 0))]
         [dy (fl- (g i 1) (g j 1))]
         [dz (fl- (g i 2) (g j 2))]
