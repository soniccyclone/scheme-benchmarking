...
it is what Chez actually does and Chez is our host and reference.

The architectural consequence: the allocator operates on the *AST*, bottom-up, in two linear
passes, with liveness as a machine word. That fits nanopass exactly. George-Appel by contrast
needs a CFG, a liveness dataflow pass, an interference graph with a sparse-set hash table, and
fixpoint iteration with program rewriting on spills. Our stage 12 is specified as linear scan;
this is a third option, and arguably the one that matches our IR shape best, since it never
leaves the tree.

Two things transfer regardless of allocator. First, `St`/`Sf` is a general technique for "does
this expression inevitably do X," which is exactly the shape stage 07 needs for hoisting (does
this body inevitably execute the check?) and stage 08 for escape (does this flonum inevitably
escape?). The `R`-for-impossible-paths trick is the non-obvious part and is reusable verbatim.

Second, the effective-leaf statistic is a fact about Scheme workloads that should bias stage
08: most activations never call anything, so their values never need to survive a call, so the
boxed/unboxed decision should favor the call-free path. The paper's throwaway suggestion —
static branch prediction assuming call-free paths are likelier, worth a consistent 2-3% — is
cheap at stage 13 and we should take it.

The interprocedural comparison scopes our ambitions. Steenkiste and Hennessy got 88% of stack
accesses combining intra- and interprocedural allocation, ~51% from the intraprocedural half.
This paper gets 72% with no interprocedural analysis at all, and works with first-class
anonymous procedures where the interprocedural approaches break down. That is the argument
against building call-graph-based register allocation for a Scheme.
...
The tak comparison in Tables 4-5 is the most interesting and most oversold part. Chez beats
`cc -O3` by 14% on tak(26,18,9) despite stack-overflow checks and worse instruction scheduling.
The authors then hand-modify both C compilers' assembly to use lazy saves and get 91% and 60%
speedups, plus a hand-coded caller-save version — a real experiment that isolates the variable
properly. It is also one call-recursion microbenchmark chosen because it "isolates the effect
of register save/restore strategies," which is another way of saying it is the best case. Do
not read 14% as Scheme-beats-C.

Table 3 has odd rows: `div-iter` shows 100% stack-reference reduction and 133% speedup, `tak`
109% — the baseline was doing something pathological. The 43% average is pulled hard by tiny
call-recursion benchmarks; the four large real programs (Compiler, DDD, Similix, SoftScheme)
sit at 30%, 47%, 26%, 22%. Plan against those.

One claim is asserted, not shown: "it can be shown that this placement is never too eager."
No proof for either algorithm, only the statement and a worked example. Probably easy, but if
we implement `St`/`Sf` we should verify it rather than inherit it.
